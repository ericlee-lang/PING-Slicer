...
"""
import re, json, os, sys, io, shutil, glob, math, struct
...
def bed_for(model, base=False):
    """機型對應的床身 STL。base=True 取家族原盤（不套機型專屬覆寫）——
    供「床身 STL 反向平移」取來源用（見 emit_offset_bed_stl）。"""
    key = max((k for k in BED_STL if model.startswith(k)), key=len)
    if not base:
        ov = BED_OVERRIDE.get(model, {}).get("bed_model")
        if ov:
            return ov
    return BED_STL[key]
...
BED_OVERRIDE = {
    ...
    #    🆕 **bed_model 改指專屬反向平移盤**（Eric 2026-08-11 夜裁「修圓盤」）＝眼驗①處置，
    #       檔案由 emit_offset_bed_stl() 於 regen 自動產生，不是手工搬的檔——詳見該函式註解。
    "FD300 關門": {"area_polygon": "rounded_triangle", "prime_y_shift": 50,
                   "bed_model": "PING_FD300_closeddoor_buildplate_model.stl"},
}
...
# ★ 床身 STL 反向平移（Eric 2026-08-11 夜裁「修圓盤」・T021 裝機眼驗①處置）
#   現象：關門機型的深色圓盤看起來往後偏、可印區像「掉」在盤外前緣。
#   根因：引擎把床身 STL 擺在 printable_area 的**外框中心**，不是床心——
#       src/slic3r/GUI/3DBed.cpp:624  Vec3d shift = m_build_volume.bounding_volume().center();
#     床形上下對稱時外框中心＝床心（圓床、矩形床皆然，所以上游從沒踩到）；
#     圓角三角形外框 Y[-100,+150] ⇒ 中心 (0,+25) ⇒ 圓盤被往 +Y 推 25mm。
#   ⚠ **純渲染缺陷**：超出範圍判定走 m_build_volume 真實多邊形（凸形精準路徑），
#     **不經過這個位移** ⇒ 切片與碰撞判定一直都是對的，只有畫面錯。
#   ⚠ **「把可印區往上拉」是無效解**：外框中心會跟著上移，圓盤跟著跑，落差恆為原值，
#     畫面完全不變，卻已經把真實可印範圍改錯（後緣會衝出 Ø300 盤面）。
#   對策：產一支機型專屬 STL＝圓盤先平移「−外框中心」，讓引擎的「+外框中心」剛好抵銷。
#   🔴 平移量**一律由 printable_area 現算**（見下方呼叫處），**絕不可手填數字**——
#     形狀再改會自動跟著變。閘門＝verify_profiles.py 的「床身盤實際落點＝原盤落點」斷言。
#   ℹ 治本解（乙案）＝在 3DBed.cpp 加 machine_model 鍵 `bed_model_offset`，但那要 build＋
#     全機型視覺回歸；Eric 0811 裁先走本案（profile 資源，不用 build）。
def _stl_read_binary(path):
    """回傳 (80B header, [50B 三角形紀錄...])。只支援二進位 STL（本專案床盤皆是）。"""
    with open(path, "rb") as f:
        head = f.read(80)
        if len(head) < 80:
            raise SystemExit("床身 STL 檔頭不足 80 bytes：%s" % path)
        n = struct.unpack("<I", f.read(4))[0]
        recs = []
        for _ in range(n):
            r = f.read(50)
            if len(r) < 50:
                raise SystemExit("床身 STL 面數與檔頭不符（讀到 %d/%d）：%s" % (len(recs), n, path))
            recs.append(r)
        if f.read(1):
            raise SystemExit("床身 STL 尾端有多餘位元組（疑似 ASCII STL）：%s" % path)
    return head, recs

def emit_offset_bed_stl(src_name, dst_name, dx, dy):
    """把 PINGDIR 下的床身 STL 平移 (dx,dy) 另存 dst_name（法線不動＝純平移）。回傳面數。"""
    src, dst = os.path.join(PINGDIR, src_name), os.path.join(PINGDIR, dst_name)
    _head, recs = _stl_read_binary(src)
    # 80B 檔頭寫成自述字串：下一個人打開檔案就知道它為什麼不置中
    hdr = ("PING bed plate: %s shifted X%+.3f Y%+.3f -- auto-generated by "
           "embed_params.py, do NOT hand-edit" % (src_name, dx, dy)).encode("ascii", "replace")[:80]
    out = bytearray(hdr.ljust(80, b"\x00") + struct.pack("<I", len(recs)))
    for r in recs:
        v = list(struct.unpack("<12fH", r))
        for k in range(3, 12, 3):        # 3 個頂點 ×(x,y,z)；索引 0..2 是法線，不動
            v[k] += dx
            v[k + 1] += dy
        out += struct.pack("<12fH", *v)
    with open(dst, "wb") as f:
        f.write(bytes(out))
    return len(recs)

def emit_offset_bed_models():
    """為每個「床形不對稱」的機型產生反向平移床盤。平移量現算自該機型的 printable_area。"""
    for model, ov in BED_OVERRIDE.items():
        if not ov.get("bed_model"):
            continue
        kind = ov.get("area_polygon")
        if kind == "rounded_triangle":
            pts = [tuple(map(float, p.split("x"))) for p in rounded_triangle_area()]
        else:
            # fail-closed：新的床形型別出現時必須明示怎麼算，不要靜默沿用舊平移量
            raise SystemExit("未知 area_polygon 型別 %r（%s）：床身 STL 平移量無從計算" % (kind, model))
        cx = (min(p[0] for p in pts) + max(p[0] for p in pts)) / 2.0
        cy = (min(p[1] for p in pts) + max(p[1] for p in pts)) / 2.0
        base = bed_for(model, base=True)
        n = emit_offset_bed_stl(base, ov["bed_model"], -cx, -cy)
        print("  床身反向平移：%s ← %s 平移 (%+.3f, %+.3f)，%d 面"
              "（抵銷 3DBed.cpp:624 的外框中心位移）" % (ov["bed_model"], base, -cx, -cy, n))
...
def main(src_base):
    ...
    # 4c-9. 床形不對稱機型的床身盤反向平移（Eric 2026-08-11 夜裁「修圓盤」）
    #       ——平移量現算自 printable_area，詳見 emit_offset_bed_stl 上方註解
    emit_offset_bed_models()
