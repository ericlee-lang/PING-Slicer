...
#  ⚠ 2026-09-17 起 ABS+SUP 的 raft_layers 改 3，本表維持 2（Eric 只點名易拆+筏層、未點名單料 _棧板 雙生；
...
def combo_overrides(combo, layer_height, nozzle):
    ...
    - Raft（分家族）：ABS+ABS 雙料(Z隙)+棧板＝2 層（2026-06-10 V3.0「最佳 ABS」定稿）、
      ABS+SUP 易拆(Z0)+棧板＝3 層（Eric 2026-09-17 實測改，見下方 ABS+SUP 區塊）、PLA 系＝0
    ...
    if combo.startswith("ABS"):
        o["raft_layers"] = "2"     # ABS+ABS 雙料(Z隙)+棧板吃這個 2；ABS+SUP 於下方區塊覆寫為 3（2026-09-17）
        # ABS 首層線寬 1.5×口徑（「最佳ABS(更新後)」定稿 0.4 噴嘴=0.6；百分比隨口徑縮放）
        o["initial_layer_line_width"] = "150%"
    if combo == "ABS+SUP":
        # Eric 2026-09-17 裁（實測效果）：易拆+筏層（本線顯示名＝易拆(Z0)+棧板）筏層 2→3；只限 ABS+SUP——
        # ABS+ABS 雙料(Z隙)+棧板 與單料 _棧板 雙生維持 2（他只點名易拆+筏層），牌 c-0917-REL-01
        o["raft_layers"] = "3"
