# -*- coding: utf-8 -*-
"""⛔ 已退役（2026-09-07，隨回報中心 #112「床貼圖改 SVG」）——請改用
   `tools/ping/make_bed_texture_svg.py`，它會一併產出主版與關門版並更新墨跡 fixture。

   為什麼退役：床貼圖改成 SVG 之後，「往床前緣平移 17mm」不再需要 PIL 搬像素，
   只要改 transform 的 `ty` 就好——更精確，也不必依賴 PIL。
   本檔仍留著是為了保存 UV 對應的推導（下面那段註解是唯一的正本），
   **但不要再執行它**：它會產出 PNG 並覆蓋 fixture 條目，把閘門指回已淘汰的資產。
