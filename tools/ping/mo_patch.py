...
NEW_ENTRIES = {
    ...
    "Parameter Difference Report": "參數差異清單",
    "Software": "軟體",
    "Profile bundle": "參數包版本",
    "Generated": "產生時間",
    "No differences found: every setting matches the system profile.": "沒有差異：所有設定都與系統母版相同。",
    "Parameter": "參數",
    "Standard value": "標準",
    "Current value": "目前設定",
    "Flag": "標記",
    "Updated by PING": "PING 已更新",
    "No effect": "未作用",
    "Flags: \"Custom\" means this value was changed from the system profile. \"Updated by PING\" means you did not change it — PING adjusted the standard later; it only appears when the report is generated on a machine whose profiles are newer than the ones the project was saved with. \"No effect\" means the value is inactive because of another setting in this project.": "標記說明：「自訂」代表這顆值被改過、與系統母版不同；「PING 已更新」代表你沒有動它，是 PING 後來調整了標準——只有在產生報告的這台機器、參數包比專案存檔當時新的時候才會出現；「未作用」代表這顆值因為本專案的其他設定而不會生效。",
    "Generated by PING Slicer. This file also carries the current settings as data so that PING support can re-compare them against the latest PING standard.": "由 PING Slicer 產生。這份檔案同時夾帶目前的設定資料，讓 PING 售服能用最新的 PING 標準重新比對一次。",
    "One or more presets have no system profile to compare against (the inheritance chain is broken, or the preset was imported from outside). Those sections are missing from this report.": "有預設找不到可以比對的系統母版（繼承鏈中斷，或這個預設是從外部匯入的）。報告中缺少那幾塊。",
    "Export parameter difference report": "匯出參數差異清單",
