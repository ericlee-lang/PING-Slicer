...
// ── 回抽態的 SET_RETRACTION／混色指令一律延到 G11 之後（`#353` 丙＋0910 擴充；說明見 hpp 宣告處）──
// 判準：任何時刻 `SET_RETRACTION …`、`M6051 …`、`M6052 …` 都不得出現在「G10 之後、下一個 G11 之前」。
int defer_in_retract_state(const std::string& gcode, std::string& out, DeferSetRetractionStats* stats)
{
    out.clear();
    ...
    // 保序**全部**暫存——不是「只留最後一行」：先改長度、再單獨改速度時，只留最後一行會把長度丟掉。
    // bool＝這行是不是 SET_RETRACTION；檔尾處置兩者不同（見函式尾）。
    std::vector<std::pair<bool, std::string>> held;
    ...
    while (true) {
        ...
        if (b - a >= 3 && gcode[a] == 'G' && gcode[a + 1] == '1' && (gcode[a + 2] == '0' || gcode[a + 2] == '1') && cmd_ends_at(a + 3, b)) {
            ...
            held.emplace_back(true, gcode.substr(start, line_end - start));   // 保留原行（含前導空白／尾 \r）
            skip_line = true;
            ++st.deferred;
        } else if (retracted && b - a >= 5 && gcode[a] == 'M' && gcode[a + 1] == '6' && gcode[a + 2] == '0' &&
                   gcode[a + 3] == '5' && (gcode[a + 4] == '1' || gcode[a + 4] == '2') && cmd_ends_at(a + 5, b)) {
            // 🔴 2026-09-10（Eric：「四料也出現像雙料一樣的回抽情況，請幫我調整安插指令的位置」）：
            // 混色指令 M6051／M6052 夾在 G10..G11 之間 ⇒ 換色發生在回抽態、G11 回吐的是舊配方的料。
            // 照片磚的 M605x 是掃 `;Z:` 層標記逐層插的，而換層標記正好落在回抽區間裡 ⇒ 每層都中。
            // ⚠ **M6050 刻意不收**：Classic 前代拿 `M6050 S1 P0` 當換刀指令用（0729 裁），
            //    把換刀延到回吐之後＝回吐的料從錯的通道出去。照片磚雙料走 M6051、四料走 M6052。
            held.emplace_back(false, gcode.substr(start, line_end - start));
            skip_line = true;
            ++st.deferred_mix;
        }
        // 其他行（SET_PRESSURE_ADVANCE／M104／M6050…）一律不動。
        if (skip_line) {
            if (nl == std::string::npos) break;
            start = nl + 1;
            continue;
        ...
        if (flush_held_after) {                     // G11 已原樣輸出 → 現在才依原順序全部吐出
            for (const auto& kv : held) { out.push_back('\n'); out += kv.second; }
            held.clear();
        }
        if (nl == std::string::npos) break;
        out.push_back('\n');
    ...
    if (!held.empty()) {
        // 檔尾／取消點仍在回抽態（沒有後續 G11），兩類的處置**不同**：
        //   • SET_RETRACTION：**不得**在這裡吐出——那等於在回抽態重發，違反判準本身。直接丟棄，
        //     只留一行註解供日後統計（Codex gpt-6-astra 2026-09-09 對抗審）。
        //   • M6051／M6052：照樣吐出——它不碰 is_retracted 旗標（丟棄的理由不成立），
        //     而取消點之後若還有列印，丟掉會讓後面整段用錯配方。
        for (const auto& kv : held) {
            if (kv.first) { ++st.dropped; continue; }
    ...
    if (stats) *stats = st;
    return st.deferred + st.deferred_mix;
}
