...
StringObjectException Print::validate(StringObjectException *warning, Polygons* collison_polygons, std::vector<std::pair<Polygon, float>>* height_polygons) const
{
...

	{
        ...
        for (PrintObject *object : m_objects) {
            ...
            // PING 2026-09-19 (c-0919-SB-01): support blocks are generated by the normal support generator only.
            if (warning && std::any_of(object->model_object()->volumes.begin(), object->model_object()->volumes.end(),
                                       [](const ModelVolume *mv) { return mv->is_support_mesh(); })) {
                StringObjectException warningtemp;
                if (! object->has_support()) {
                    warningtemp.string  = L("Support meshes are used but support is not enabled. Please enable support.");
                    warningtemp.opt_key = "enable_support";
