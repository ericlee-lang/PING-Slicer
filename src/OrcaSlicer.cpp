...
std::map<int, std::string> cli_errors = {
    ...
    {CLI_NO_SUITABLE_OBJECTS, "One of the plate is empty or has no object fully inside it. Please check that the 3mf contains no empty plate in PING Slicer before uploading."},
    {CLI_VALIDATE_ERROR, "There are some incorrect slicing parameters in the 3mf. Please verify the slicing of all plates in PING Slicer before uploading."},
    ...
    {CLI_FILAMENT_NOT_MATCH_BED_TYPE, "Filaments are not compatible with the plate type. Please verify the slicing of all plates in PING Slicer before uploading."},
    {CLI_FILAMENTS_DIFFERENT_TEMP, "The temperature difference of the filaments used is too large. Please verify the slicing of all plates in PING Slicer before uploading."},
    {CLI_OBJECT_COLLISION_IN_SEQ_PRINT, "Object conflicts were detected when using print-by-object mode. Please verify the slicing of all plates in PING Slicer before uploading."},
    {CLI_OBJECT_COLLISION_IN_LAYER_PRINT, "Object conflicts were detected. Please verify the slicing of all plates in PING Slicer before uploading."},
    {CLI_SPIRAL_MODE_INVALID_PARAMS, "Some slicing parameters cannot work with Spiral Vase mode. Please solve the issue in PING Slicer before uploading."},
    ...
    {CLI_SLICING_ERROR, "Failed slicing the model. Please verify the slicing of all plates on PING Slicer before uploading."},
    {CLI_GCODE_PATH_CONFLICTS, " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest PING Slicer. If the file slices normally in PING Slicer, try moving the wipe tower further from other models, as we use more conservative parameters for it during upload."},
    {CLI_GCODE_PATH_IN_UNPRINTABLE_AREA, "Found G-code in unprintable area of multi-extruder printers after slicing. Please make sure the 3mf file can be successfully sliced in the latest PING Slicer."}
};
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into PING Slicer process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a PING Slicer process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the PING Slicer process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"PING Slicer is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using PING Slicer.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes PING Slicer "
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    ...
    boost::nowide::cout
        ...
        << std::endl
        << "Usage: ping-slicer [ OPTIONS ] [ file.3mf/file.stl ... ]" << std::endl
        << std::endl
        << "OPTIONS:" << std::endl;
