...
int MoonrakerPrinterAgent::handle_request(const std::string& dev_id, const std::string& json_str)
{
    ...
    if (json.contains("print") && json["print"].contains("command")) {
        ...
        // Handle gcode_line command - this is how G-code commands are sent from PING Slicer
        if (cmd == "gcode_line") {
    ...
    while (!ws_stop.load() && retry_count < max_retries) {
        ...
        try {
            ...
            ws.set_option(websocket::stream_base::decorator([&](websocket::request_type& req) {
                req.set(http::field::user_agent, "PING Slicer");
                if (!api_key.empty()) {
            ...
            identify["params"]["client_name"] = "PING Slicer";
