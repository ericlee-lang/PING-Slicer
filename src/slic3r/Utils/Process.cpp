...
// For starting another PING Slicer instance on OSX.
...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance)
{
	...
	path += "\\";
	path += (instance_type == NewSlicerInstanceType::Slicer) ? "ping-slicer.exe" : "bambu-gcodeviewer.exe";
	...
	// Don't call with wxEXEC_HIDE_CONSOLE, PING Slicer in GUI mode would just show the splash screen. It would not open the main window though, it would
...
#if defined(__APPLE__)
	{
		...
		//bin_path = bin_path.parent_path() / "PING Slicer";
...
#else // Linux or Unix
	{
		...
		if (args.empty()) {
			...
			my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "ping-slicer" : "bambu-gcodeviewer")).string();
