...
std::shared_ptr<HttpServer::Response> HttpServer::bbl_auth_handle_request(const std::string& url)
{
    ...
    if (!auth_code.empty()) {
        ...
        const std::string message = login_ok
            ? "You can return to PING Slicer. This window will close automatically."
            : "Something went wrong. Please return to PING Slicer and try again.";
...
void HttpServer::ResponseRedirect::write_response(std::stringstream& ssOut)
{
    const std::string sHTML =
        ...
        "<p>You can return to PING Slicer. If your browser does not redirect automatically, use the button below.</p>"
