...
SwitchButton::SwitchButton(wxWindow* parent, wxWindowID id)
    ...
    , thumb_color(std::pair{0xEA4E16, (int) StateColor::Checked}, std::pair{0xD9D9D9, (int) StateColor::Normal})
{
...
void SwitchBoard::doRender(wxDC &dc)
{
	...
	/*left*/
    if (switch_left) {
        is_enable ? dc.SetBrush(wxBrush(wxColour(234, 78, 22))) : dc.SetBrush(disable_color);
	...
	/*right*/
    if (switch_right) {
        if (is_enable) {dc.SetBrush(wxBrush(wxColour(234, 78, 22)));
        } else {dc.SetBrush(disable_color);}
