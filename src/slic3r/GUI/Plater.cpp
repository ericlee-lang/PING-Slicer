...
void Sidebar::priv::flush_printer_sync(bool restart)
{
    ...
    //btn_sync_printer->SetBackgroundColorNormal((*counter_sync_printer & 1) ? "#F8F8F8" :"#EA4E16");
...
{
    StateColor not_synced_colour(std::pair<wxColour, int>(wxColour("#EA4E16"), StateColor::Normal));
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        */
        ...
        StateColor btn_sync_bd_col(
                std::pair<wxColour, int>(wxColour("#EA4E16"), StateColor::Pressed),
                std::pair<wxColour, int>(wxColour("#EA4E16"), StateColor::Hovered),
    ...
    text_ctrl->Bind(wxEVT_SET_FOCUS, [this](wxFocusEvent& e) {
        ...
        p->m_search_bar->SetBorderColor(wxColour("#EA4E16"));
...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (!unknowns.empty()) {
        MessageDialog dlg(this,
            _L("There are some unknown or incompatible filaments mapped to generic preset.\nPlease update PING Slicer or restart PING Slicer to check if there is an update to system presets.") + detail,
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
    if (!sync_color_only) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3MF is not supported by PING Slicer, loading geometry data only."), _L("Load 3MF"));
                    }
                    ...
                    //             show_info(q, _L("The 3MF was generated by an old PING Slicer, loading geometry data only."), _L("Load 3MF"));
                    //         else
                    //             show_info(q, _L("The 3MF is not supported by PING Slicer, loading geometry data only."), _L("Load 3MF"));
                    //     }
                    ...
                    //     }
                    // }
                    // Orca: check if the project is created with PING Slicer 2.3.1-alpha and use the sparse infill rotation template for non-safe infill patterns
                        ...
                        if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                            ...
                            if (!is_safe_to_rotate) {
                                wxString msg_text = _(
                                    L("This project was created with an PING Slicer 2.3.1-alpha and uses "
                                ...
                                msg_text += "\n\n" +
                                            _(L("Would you like PING Slicer to automatically fix this by clearing the rotation template settings?"));
                        ...
                        load_config = false;
                        // 照片磚工作室產生的 3MF 天生不帶專案設定（沿用當前機型/製程選擇），不是舊版檔——不跳警示
                        bool ping_photo_tile_3mf = false;
                        if (model.model_info) {
                            auto app_it = model.model_info->metadata_items.find("Application");
                            ping_photo_tile_3mf = app_it != model.model_info->metadata_items.end() &&
                                                  boost::starts_with(app_it->second, "PING-PhotoTile");
                        }
                        if (!ping_photo_tile_3mf)
                            show_info(q, _L("The 3MF file was generated by an old PING Slicer version, loading geometry data only."), _L("Load 3MF"));
                    }
                    else if (!load_config) {
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "PING Slicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to PING Slicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        if (device_page) {
            ...
            content = wxString::Format(
                _L("PING Slicer can't connect to %s. Please check if the printer is powered on and connected to the network."), printer_name);
        }
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on PING Slicer(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
