...
wxBoxSizer* PreferencesDialog::create_item_link_association( wxString url_prefix, wxString website_name)
{
    ...
    wxString tooltip = _L("Associate") + " " + url_prefix + ":// " + _L("with PING Slicer so that it can open models from") + " " + website_name;
...
void PreferencesDialog::create_items()
{
    ...
    auto item_single_instance  = create_item_checkbox(_L("Allow only one PING Slicer instance"),
    #if __APPLE__
    ...
    #else
            _L("If this is enabled, when starting PING Slicer and another instance of the same PING Slicer is already running, that instance will be reactivated instead."),
    #endif
            "single_instance");
    ...
    auto item_auto_reslice = create_item_auto_reslice(
        ...
        _L("If enabled, PING Slicer will re-slice automatically whenever slicing-related settings change."),
    ...
    g_sizer->Add(create_item_title(_L("Associate files to PING Slicer")), 1, wxEXPAND);

    auto item_associate_3mf    = create_item_checkbox(_L("Associate 3MF files to PING Slicer"), _L("If enabled, sets PING Slicer as default application to open 3MF files.") , "associate_3mf");
    g_sizer->Add(item_associate_3mf);

    auto item_associate_drc = create_item_checkbox(_L("Associate DRC files to PING Slicer"), _L("If enabled, sets PING Slicer as default application to open DRC files."), "associate_drc");
    g_sizer->Add(item_associate_drc);

    auto item_associate_stl    = create_item_checkbox(_L("Associate STL files to PING Slicer"), _L("If enabled, sets PING Slicer as default application to open STL files.") , "associate_stl");
    g_sizer->Add(item_associate_stl);

    auto item_associate_step   = create_item_checkbox(_L("Associate STEP files to PING Slicer"), _L("If enabled, sets PING Slicer as default application to open STEP files."), "associate_step");
    g_sizer->Add(item_associate_step);

    //// ASSOCIATE > WebLinks
    g_sizer->Add(create_item_title(_L("Associate web links to PING Slicer")), 1, wxEXPAND);
