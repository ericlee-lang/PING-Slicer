...
wxBoxSizer* NetworkTestDialog::create_info_sizer(wxWindow* parent)
{
	...
	text_version_title = new wxStaticText(this, wxID_ANY, _L("PING Slicer Version:"), wxDefaultPosition, wxDefaultSize, 0);
...
wxBoxSizer* NetworkTestDialog::create_content_sizer(wxWindow* parent)
{
	...
	btn_link = new Button(this, _L("Test PING Slicer (GitHub)"));
    btn_link->SetStyle(ButtonStyle::Regular, ButtonType::Window);
	grid_sizer->Add(btn_link, 0, wxEXPAND | wxALL, 5);

	text_link_title = new wxStaticText(this, wxID_ANY, _L("Test PING Slicer (GitHub):"), wxDefaultPosition, wxDefaultSize, 0);
...
void NetworkTestDialog::start_all_job_sequence()
{
	m_sequence_job = new boost::thread([this] {
        ...
        if (m_closing) return;
		start_test_url(TEST_ORCA_JOB, "PING Slicer(GitHub)", "https://github.com/OrcaSlicer/OrcaSlicer");
		if (m_closing) return;
