...
void GLGizmoSeam::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    for (int i = 0; i < tool_ids.size(); i++) {
        ...
        if (m_current_tool == tool_ids[i]) {
            ImGui::PushStyleColor(ImGuiCol_Button,          ImVec4(0.918f, 0.306f, 0.086f, 0.25f));  // ORCA use orca color for selected tool / brush
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered,   ImVec4(0.918f, 0.306f, 0.086f, 0.25f));  // ORCA use orca color for selected tool / brush
            ImGui::PushStyleColor(ImGuiCol_ButtonActive,    ImVec4(0.918f, 0.306f, 0.086f, 0.30f));  // ORCA use orca color for selected tool / brush
