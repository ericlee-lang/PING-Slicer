...
---
### ✅ 色彩校正兩件落地＋黑底退化定罪推進・第十棒（2026-08-07・牌 c-0807-PT-22）

**一句話**：第九棒交辦的 🔴 待辦 1（色彩校正兩件）**做完了**，全程免 build；
待辦 2（黑底配色退化）**推翻了第九棒的推測**並把缺口縮小到「只差一個素材」。

> 🧭 **下一棒起跑先讀這三行**：
> ① **不要照症狀去改建議配色演算法**——現行碼在 6 種黑底輸入上**全部都挑得到暗色**，
>    Eric 那次的失敗**重現不出來**。查證台在 `照片磚_配色查證/`（含 README 記錄全部證據）。
> ② **色彩校正的東西都在 `照片磚_色彩校正/`**，規格 §6 已補「實作現況」節（紀錄，非新規則）。
> ③ **開發線只多一顆 commit**（外層 repo `b92d93b`）；**PING-Slicer repo 本棒全程未寫**，
>    出貨線未碰、不押 T、`%APPDATA%` 未碰、**未 build**。

**✅ 做了什麼**

1. **四料 48 格校正塊產生器**（`make_calib_quad_3mf.py`，規格 R6-2／R6-6 A 案）：
   正面 80×60mm、厚 10mm、48 格＋洗料柱＝**49 支零件**（≤Orca 上限 64）。
   **八階比例直接抄引擎量化式** `round((1-s/7)*100)`＝`100/86/71/57/43/29/14/0`
   ——校正表的鍵必須跟引擎**實際會吐**的比例逐字相同，否則量了 70% 而引擎永遠只印 71%＝白量。
   附**自我驗證**：照 C++ `parse_photo_part_name` 規則逐條複驗（id 四處一致／尾端 A B C D 且
   和＝100±0.5／有 `#RRGGBB` 預覽色／Z 貼板），**49 支全過**。
2. **取色可操作原型**（`取色原型.html`，規格 R6-3；Eric 鐵則「人機操作功能＝可操作原型先行」）：
   單檔離線、雙擊即開。點四角 → 透視解 homography → 切 8×6 格 → 取每格中心 60% 區平均 →
   列「配方／理論色／量到色／ΔE」；**取樣點可直接拖曳＝吸管備援**。
   **內建合成範例照片**（`make_sample_photo.py` 產，含透視／打光不均／層紋／雜訊）
   ⇒ 不必等實體印出就能走動線（AI-first：不讓 Eric 先印一塊才能試 UI）。
   **實測（不是看碼）**：48 格對位正確（R1C1＝純A、R1C8＝純B、R4C1＝純B、R6C8＝純D）、
   平均 ΔE 2.3；**打光一致性檢查抓得到**我刻意做的左亮右暗（純 B 料格之間 ΔE 9.5）＝真陽性非假警報；
   FBK-11 匯出擋門會講「還缺右下、左下沒點」；一屏不捲動（初版實測溢出 3px、已改 flex 修掉）。
   ⚠ **畫面長相未眼驗**：Browser 面板在本 session 無法顯示 ⇒ 截圖不到。功能與版面用量測驗過
   （rect／canvas 像素／scrollHeight），但**視覺以 Eric 實走為準**。
3. **黑底配色退化（待辦 2）——推翻第九棒推測，缺口縮小**：
   - Eric 放進 `照片磚_K階測試\` 的 `螢幕擷取畫面 2026-08-07 065100.png`（450KB 粉紅貓／黑底；
     第九棒看到時 0KB）**跑出來是健康的**：`白／#df719c 粉／#1a1615 近黑／#868184 灰`。
     再掃 6 種變體（只留畫布／黑底壓純黑／壓暗／提亮／低對比／純合成）**全部挑得到暗色**。
   - **排除「跑到舊碼」**：試用包 `D:\PING-Slicer-dev-portable\resources\web\phototile\` 的
     `index.html`／`engine.js` 與 repo **md5 完全相同**。
   - **從 Eric 報的數字反推**（線性空間解混色）：`67/33` 只有 **K=4** 才會出現（K=8 是
     100/86/71/57/43/29/14/0）⇒ 他那次色階數調到 4；以 A=`#f2f0eb` 反推
     `A67B33=#E4CAC7` ⇒ 料B≈`#C43A47`、`A67C33=#E3CAC5` ⇒ 料C≈`#C03A2C`
     ——**兩支都是 L\*≈42 的紅**，最深只到 L\*42 ⇒ **「整盤無暗色」與「ΔE 48」兩個症狀同時對上**，
     但**貓圖跑不出這兩支紅**。
   - ⇒ **第九棒的「pool 群數不足→退化階梯」不成立**（退化階梯每次把 L\* 推開 28，不會生兩支相近色）。
   - **還缺**：失敗那次真正餵進去的圖檔，**或**那次畫面「本次料色：A=…｜B=…｜C=…｜D=…」那一行。

**🟠 交下一棒的待辦（依重要度；1、2 是新的，3~7 承接第九棒未辦）**

1. 🔴 **等 Eric 走完取色原型**再決定要不要接進工作室 UI（照鐵則：動線先驗、才寫底層碼、最後接 UI）。
2. 🔴 **黑底配色**：拿到素材再查，**沒素材不要動演算法**。
3. 🟠 **最小刀（出貨線 V3.6.1）**——⚠ 出貨線已被 MAT 線推進到 **`b824a059`／bundle 91**
   （第九棒記的「獨有 12 顆」是對 `102fc948` 算的）⇒ **動手前必須重抓 tip、重算分歧、先跟該線協調**。
   建議仍拆兩批：第一批（四條不再靜默覆寫＋換料清單不寫死白＋FF600 profile）／第二批（配色本體，等 2 修完）。
   ⚠ 推 `release/*` 會觸發真 build ⇒ 發車等 Eric。
4. 🟠 **照片磚工作室自動安裝機型**（四色→FF800＋FF600、雙色→FD300）；屬 C++、**要 build**。
5. 🟡 **混色塊（洗料柱）與照片磚分開**。⚠ **洗料柱 ≠ 色彩校正塊**；且**校正塊的柱高已定＝必須等於
   整塊高、不可縮**（柱矮於某層 ⇒ 那層以上沒被洗過、量值作廢），別跟「照片磚的柱要不要縮」混為一談。
6. 🟡 **建議配色 SSOT 統一**（Eric 裁「等進版後再說」；統一前**改一邊必須改另一邊**——
   本棒實查 `index.html:937-993` 與 `engine.js:366-412` 目前是**同碼**）。
7. ⚪ 承接第八棒未辦：4GB 帽下切片 AV 硬崩／閘門收攤 `0xc0000374`／閒置預熱 332.5MB／閘門① `PASS_WARMED`。

**🧰 本棒沉澱**
- `照片磚_核心規格.md` **§6 新增「實作現況」節**（紀錄性質）：含 3 個實作細節與 **2 件等 Eric 裁**
  （純白格白平衡→**結構上已自動滿足**、每支料的純色格各有 3 個；每格 1cm 夠不夠等實印）。
- 🆕 **`照片磚_配色查證/`**（含 README）：建議配色的 Node 查證台。
  **教訓**：第九棒把同款腳本放 scratchpad ⇒ 換棒即消失、本棒重寫一次。**查證工具要進專案，不放暫存區。**

**線況**：外層 repo（`D:\dev\2026claude`）新增 1 顆 `b92d93b`（校正塊產生器＋取色原型＋查證台＋規格 §6）；
**`PING-Slicer` repo 本棒只改本檔 `PING_HANDOFF.md`**——`src/`、`resources/` 與任何切片行為
**零改動**，出貨線 `release/v3.6` 全程未碰；**不押 T**；
**`%APPDATA%`／Eric 正式版／試用包全程未改**（只讀 md5 比對）；**未 build**。
