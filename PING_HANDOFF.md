...
- ✅ **r6 build 綠＋portable 已換裝（2026-07-14）**：三批（`c8c3e751`＋`4a3fa192`＋`cb10b456`）cherry-pick 上 release/v3.5.5＝`0673b585`/`92db6578`/`b5137817`＋版號 `34194a64`（tip）→ run **29296142180**。第一輪 Windows 在 cpack 缺 NSIS 紅＝**chocolatey 社群源 504**（編譯含 PartPlate.cpp 全綠、純 infra；choco 防呆重試已進 dev `4063d302`）→ rerun --failed 全過。`D:\PING-Slicer-portable`＝**r6**（bundle v49 全項驗過），備份 `-old-v355r5`；**%APPDATA% 版號已抬 50**（bundle 49<50＝DL1016 紅線維持）。G 槽/官網檔未動（等 Eric 裁）。cherry-pick 時照片磚 2 機檔 DU 衝突按「release 無照片磚」原則 git rm；release 驗證全過（支撐四值/黃金不蓋/FD300 弧/FP300 弧/FP200 不在通用版/照片磚 0/py_compile）。**版號：bundle 48→49、%APPDATA% 49→50**（維持 bundle<appdata 嚴格小於＝DL1016 紅線；r5 用戶 48→49 會吃到更新）。綠了照慣例換 portable（現版＝r5，備份名照 -old-v355r5）。
