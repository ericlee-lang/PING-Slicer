...
---
### ✅ 色彩校正兩件＋最小刀兩批進版＋V3.6 測試包交付・第十棒收工（2026-08-07~08・牌 PT-22~28）

**一句話**：前半做完色彩校正兩件（產生器＋取色原型，經 Eric 實走修一輪）；
後半 Eric 裁「1→4→2」，**最小刀兩批＋C++ 自動安裝機型全部進版並推上遠端、四平台編譯全過**；
**黑底退化案的前提本身出現疑點**，不要照舊稿追。

> 🧭 **下一棒起跑先讀這五行**：
> ① **黑底退化案的前提動搖了**：Eric 本棒明說「**料B≈料C 不懂這個**」。六種黑底輸入實測**現行碼全部健康**、
>    重現不出來。**不要憑症狀改演算法**——查證台在 `照片磚_配色查證/`，但**先問 Eric 有沒有這回事**。
> ② **最可能的解釋已找到（未證實）**：**舊法會吐出逐字元相同的料**＝「兩支料一樣」的形狀，
>    而 **Eric 的正式版用的就是舊法**（試用包才是新法）⇒ 他很可能是在**正式版**看到的。
>    ⚠ 對不上的是他報的兩格差 (1,0,2) 而非完全相同 ⇒ **只差他一句「那次在哪支軟體看的」就能結案**。
> ③ **測試包已交付 `D:\PING-Slicer-v36-ci`**（bundle 94，含本棒全部）。**`90d28ecb` 的執行期驗證還沒做**
>    ——AI 的 GUI 權限被拒，改由 Eric 實測代驗（`使用說明.html` 已寫明檢查三點）。**他若回報結果要收進來。**
> ④ **校正塊 3MF 已在 Eric 手上**（FF600／0.6／白黑紅藍）。**所有數字目前都是合成照片量的**——
>    對位門檻 20、每格 1cm、每階厚度，**實體印出來拍照後要重量一次**。
> ⑤ **出貨線收工時已被別線推進到 `5678384a`**（Flatpak 改手動＋Unit Tests 修紅燈）⇒ **動之前先 pull**。

**✅ 做了什麼**

1. **色彩校正兩件（規格 §6）**——**48 格校正塊產生器** `make_calib_quad_3mf.py`（A 案 8 階沿 X×6 對疊 Z＝
   8×6×1cm＋洗料柱共 49 支零件；八階比例直接抄引擎量化式 100/86/71/57/43/29/14/0；自我驗證照 C++
   `parse_photo_part_name` 規則逐條複驗）＋**取色原型** `取色原型.html`（單檔離線、內建合成範例照，
   不必等實體印出就能走動線）。**Eric 實走後修四項**（見下）。
2. **最小刀第一批**（`21471ec0`＋`6f6ed54f`）——①**四條靜默覆寫**改掉（改尺寸／切雙料四色／切平躺垂直／
   勾洗料柱都會把使用者調好的料色整組洗掉）②**換料清單不再寫死「白」**③**FF600 同進照片磚**（bundle 92→93）。
   四條**逐條行為實測**（起 http server 真觸發 DOM 事件比對 `slots`）；regen **零非預期漂移**
   （既有 `setting_id` 變動 0 行，靠保留號段 PINGM/PINGP 900）。
3. **最小刀第二批**（`7046d95d`）——建議配色演算法本體移植，兩線該分支**逐字元相同**。
   **A/B 實測**（`照片磚_配色查證/ab_old_vs_new.js`）：家族不足圖 舊 ΔE 11.173／色數 8／**三支料相同** →
   新 3.211／22／無重複；黑底貓圖 舊 6.872／21／**兩支重複** → 新 5.245／40／無重複。
4. **照片磚工作室自動安裝機型**（`90d28ecb`，**C++**）——掛在 `open_photo_tile()`（首頁＋拖圖兩條路都涵蓋），
   走 `AppConfig::set_variant()` → `PresetBundle::load_installed_printers()`（與設定精靈同一條路徑）；
   比對條件沿用既有 resolve 的取向（`printer_model` 含「同進照片磚」、不寫死機名），實資料命中 **3 機型／8 口徑**。
   ✅ **四平台編譯全過**（run 31153175195）｜🔴 **執行期未驗**。
5. **V3.6 測試包**（`D:\PING-Slicer-v36-ci`）——CI portable artifact＋自帶 `data_dir` 隔離。
   **隔離實測**：正式版 conf mtime 與 md5 **完全未變**、`data_dir` 0→373 檔；`START.bat` 純 ASCII＋CRLF（SOP §5）。

**🧾 Eric 本棒實走取色原型抓到的缺陷（已修四項）**

> 原話：「**我看懂了，左上角是要點那個角，不是左上的色塊**」「**這個是不是有點危險？**」

他把「左上角」理解成**左上那一個色塊** ⇒ 格線縮進去一格、ΔE 飆到 57.9，**但畫面看起來仍像正常讀到值**
＝**會靜默產出一份錯的校正表**。四修：①文案改「點整塊的**四個外角**」＋附 ✓／✗ 示意圖
②**四角可拖曳即時修正**（原本只有「重來」＝逼人重點四次）③**對位自動偵測＋匯出擋一次**
④清四角時一併清表格（舊值留著會被當成現在的讀值）。

⚠ **對位判準第一版猜錯、量了才改**：原用「格內色差」**抓不到**（縮一格時取樣區仍落在單一色塊，
只是落在**錯的**色塊）；改用「**同一支料的 3 個純色格必須彼此相同**」（實體物件保證，與色彩理論無關）
後分離乾淨：正確 9.5／歪 1/4 格 10.4／歪半格 49.2／**點到色塊角 90.4**／左右顛倒 106.9，門檻取 20。

**🟠 交下一棒的待辦（依重要度）**

1. 🔴 **收 Eric 的兩個回報**：①測試包裡「自動安裝機型」有沒有跳通知、數字是不是 **3 種／8 口徑**、
   清單是否真的多出 FD300／FF600／FF800 ②「料B≈料C」那次是在哪支軟體看的。**兩個都只差他一句話**。
2. 🔴 **校正塊實印**——印出來拍照後用取色原型讀 48 格，把**對位門檻（現 20）、每格 1cm 夠不夠、
   每階要多厚才穩定**三個數字**從合成換成實測**。
3. 🟠 **規格 §6 兩個小裁示待 Eric**：①「同料純色格一致性檢查」升不升條文（已實作已驗證）
   ②白平衡要不要進規格（原型上可切換、預設關；範例照上只把 ΔE 2.30→2.21，因該張失真是漸層不是整體偏色）。
4. 🟡 **洗料柱（照片磚那支）要多高**——等 Eric／工程端給實務值。
   ⚠ **校正塊那支已定＝必須等於整塊高、不可縮**（柱矮於某層 ⇒ 那層以上沒被洗過、量值作廢），兩件事不要一起砍。
5. 🟡 **建議配色 SSOT 統一**（Eric 裁「等進版後再說」）。統一前**改一邊必須改另一邊**（本棒兩線已逐字元一致）。
6. ⚪ 承接未辦：4GB 帽下切片 AV 硬崩／閘門收攤 `0xc0000374`／閒置預熱 332.5MB 要不要縮條件／閘門① `PASS_WARMED`。

**🧾 本棒被推翻／澄清的三件（別再沿用舊稿）**

- ❌ **「`C:\Program Files\PING slicerV3.0` 會反蓋 `%APPDATA%`、吃掉 DL1016」＝不成立**。
  實查：它用 **`%APPDATA%\PING-Slicer\3.0.0\`**（有連字號＋版本子夾），現行線是 **`%APPDATA%\PINGSlicer\`**，
  天然隔離。**決定性證據＝那支早就被開過了**（舊 data dir 已展開 `02.00.00.00`），而現行線至今完好。
  ⇒ 要刪可以（舊世代、約 95MB），理由是「用不到」，**不是**「會吃掉 DL1016」。
- ⚠ **黑底退化案**：見上面 ①②，**前提待 Eric 確認**；在那之前不要動演算法。
- ✅ **兩盞 CI 紅燈已被別線修掉**（`9b661748` Unit Tests 零測試假綠＋403 染紅、`5678384a` Flatpak 改手動）。
  本棒的定罪方法仍有參考價值：**看 job／step 層級，不要看總表顏色**。

**🧰 本棒沉澱（不要只讀 handoff）**
- 🆕 `照片磚_色彩校正/README.md`——校正塊與取色原型用法、**Eric 實走回饋四修**、**對位偵測實驗表**。
- 🆕 `照片磚_配色查證/ab_old_vs_new.js` ＋ README 的 A/B 表——舊法 vs 新法可重跑量測台。
- `照片磚_核心規格.md` §6 補「對位自動偵測」一段（升不升條文等 Eric）。
- UX 筆記已 append `D:\dev\2026claude\UX筆記收件匣.md`（方位詞歧義／結構性防呆／就地微調三條通則候選）。

**🔧 治理面：本棒踩到並修好的三個坑（下一棒照做）**
- **push 前要重抓 tip 再比對清單**——第一次發車實際推了**四顆**（第三方 commit 落在「查清單」與「push」之間）。
  第二次發車已改：fetch→比對→**數量不符就停手**，結果零意外。
- **查他線有沒有掛牌，要連 `RELEASED` 一起看**——只篩 `CLAIMED` 會把「當時有權動、現已銷牌」的紀錄濾掉。
- **比版號前先確認兩邊在不在同一個命名空間**——V3.0「地雷」就是只比版號沒比 data dir。
  三者同源：**拿當下的快照去推斷過去／別處的狀態**。

**線況**：出貨線 `release/v3.6` 本棒推兩次（`07be2a38`→`7046d95d`），**收工時已被別線推到 `5678384a` 且遠端同步**；
開發線 `ping/v3.5` **本棒全程未碰**；**不押 T**；`%APPDATA%` **只讀不寫**
（收工實測 bundle **95**、DL1016 4 檔在，是 MAT 線抬的）；測試包獨立 `data_dir`；
背景進程零殘留；牌 **PT-22~28 七張全銷**。
---
### ✅ 待辦清空＋試用包交付＋色彩校正立規格・第九棒收工（2026-08-07・牌 c-0806-PT-20）

**一句話**：Eric 令「待辦照順序做」⇒ 第八棒六項待辦清掉五項；**開發線 Portable 試用包已交付並經 Eric 實走**，
他當場回報一個真缺陷、提一個新需求、下了**色彩校正六裁**（已立成規格 §6）。

> 🧭 **下一棒起跑先讀這四行**：
> ① **色彩校正規格已定案**：`照片磚_核心規格.md` **§6**（六條 R6-x 全裁完，48 格排法＝**A 案**）。
>    ⚠ **不要照 0807 稍早的「校正是必要條件」實作硬閘門**——Eric 當天就改掉了，R6-5 明文推翻。
> ② **最優先的下一步＝色彩校正兩件**：校正塊 3MF 產生器（8階×6對＝48 格，8×6×1cm）
>    ＋取色原型（上傳照片→點四角自動切格）。**取色是人機操作功能 ⇒ 照 Eric 鐵則「可操作原型先行」**。
> ③ **建議配色在黑底照片上會退化**（Eric 實測 ΔE 48、料B≈料C），**卡在等他那張 `image.png`**——
>    合成案在 node 重現不出來（有挑到 L\*10 暗色）⇒ 差別在輸入，沒有原圖查不下去。
> ④ **試用包在 `D:\PING-Slicer-dev-portable`**，組法已沉澱成 `SOP_開發線Portable試用包.md`。

**✅ 做了什麼**

1. **推 3 顆 commit**（`9cb7a2d2..1fdc4f4a`）——⚠ **修正舊記載**：推開發線**會**觸發 `ping_verify.yml`
   （12~16 秒、兩 run 皆綠），只是不觸發 `build_all.yml`。「推 ping/v3.5 不觸發 CI」這句話不精確。
2. **開發線 Portable 試用包**（`D:\PING-Slicer-dev-portable`，**沒有重 build**——實查「無任何 .cpp/.hpp 比
   `OrcaSlicer.dll`(0805 07:05) 新」）：40 個 exe/dll＋resources 2,915 檔＋自帶 `data_dir`＋
   `使用說明.html`（PING CIS）＋ASCII 啟動器（會擋「正式版已在跑」）。
   **隔離是實測的**：`%APPDATA%\PingSlicer\PingSlicer.conf` mtime `12:52:18 → 12:52:18` 未動、
   `data_dir` 0→402 檔；啟動後**截圖眼驗**渲染畫面。↳ 已沉澱 `SOP_開發線Portable試用包.md`。
3. **V3.6.1 整合評估**——**推翻原本「兩線差 42 顆」的理解**：是**雙向分歧**。
   開發線獨有 39 顆＋**12 支 `PhotoTile*.cpp/hpp`**＋`engine.js/engine.html/engine_protocol.js`；
   **出貨線獨有 12 顆照片磚 commit**（0715–0729 Eric 裁過的產品功能：接縫 enforcer／P2A 縫遮／
   V 溝內埋再移除／口徑濾除／感知均勻色階／工作室自動選機），而且**出貨線一支 PhotoTile C++ 都沒有**
   （純網頁層）；分岔點 `b9165c0d` ＝ 2026-07-02。
   ⇒ 🔴 **用覆蓋方式把開發線照片磚搬過去會回退掉 Eric 0719–0729 一整批裁示。**
   **Eric 裁「最小刀先做、SSOT 等進版後再說」**；我建議再拆兩批（見待辦 1）。
   實查佐證：出貨線 `index.html` **同樣三個病灶都在**（複製充數×1／`if(img) suggest()`×4／換料清單寫死白×1）
   ⇒ 最小刀移植得過去。
4. **④平躺↔垂直**：靜態驗證通過（handler 已先跑 `syncOrientUI(); renderSlots();`、
   `simulate()` 自己依 `params.orient` 分派 ⇒ 沒掉副作用；與已實測的 modeSeg 同一形狀）。
   ⚠ **仍非活體實測**——Browser 工具在本機載 `file://` 是空頁（0×0），GUI 操控權限被 Eric 拒。
5. **⑤FF800 只列 0.4mm**：**資料層四項全部排除**（機型宣告 `0.4;0.6;1.0`／三支 preset 檔在／
   PING.json 三筆註冊／`instantiation` 皆 true，**開發線與正式版 bundle 89 都一樣**）
   ⇒ 若真只列 0.4 就是顯示層。**UI 未眼驗**（同上，權限被拒）。
6. **首頁收起「雙料照片」入口**（`704e275f`，Eric 0807 裁「不要放」）：查證＝photo-mixer 是
   `a11ee870`（07-02）一次性 MVP、**1 顆 commit** 至今沒動；照片磚 29 顆、活到 0805。
   只收入口、檔案全留、註解寫復原法。**試用包已同步**。
7. **色彩校正立成規格**（`照片磚_核心規格.md` §6，commit `ae71e84`＋A 案定案）。

**🧾 Eric 0807 六裁（規格 §6 正本，這裡只列索引）**
R6-1 校正只綁 A/B/C/D **線材**，不綁機型/口徑/層高（已知取捨已誠實註記）｜
R6-2 雙料＝8 階一條校正條、四料＝6 對×8 階＝**48 格**校正塊，每格先抓 **1cm×1cm**，垂直疊加印｜
R6-3 回讀＝上傳照片**點四角自動切格**，吸管當備援，不做自動辨識｜
R6-4 換一支只失效**含該支的三組**｜R6-5 **不做硬閘門**（推翻同日稍早的「必要條件」提案）｜
R6-6 48 格排法＝**A 案**（8 階沿 X、6 對疊 Z ⇒ 8×6×1cm、48 零件；否決 B 案 48cm 高）。

**🟠 交下一棒的待辦（依重要度）**
1. 🔴 **色彩校正兩件**（規格 §6 已定案，可直接動手）：①校正塊 3MF 產生器 ②取色原型
   （**可操作原型先行**）。⚠ 我提過但**還沒進規格**的一條：拍照會整體色偏，建議在校正塊上留
   **一格純 A 料（白）當白平衡參考**先把色偏拉正——**等 Eric 點頭才寫進 §6**。
2. 🔴 **建議配色黑底退化**——等 Eric 那張 `image.png`（要他丟到 `照片磚_K階測試\`；0807 那次
   資料夾裡只有 0KB 的螢幕擷取檔）。已知：症狀是**料B≈料C**（`A67B33=#E4CAC7` vs `A67C33=#E3CAC5`）
   ＋整盤無暗色 ⇒ 形狀對應**退化階梯**（pool 群數不足→同色相推亮度），**問題在分群不在挑色**。
   重現腳本 `scratchpad/repro_suggest.js`（抄 `index.html:852-993`）：合成黑底案**有**挑到 L\*10 暗色
   ⇒ 差別在輸入。
3. 🟠 **最小刀（出貨線 V3.6.1）**——Eric 已裁「先做」。建議拆兩批：**第一批**（與配色無關、現在可移）＝
   四條不再靜默覆寫＋換料清單不寫死白＋FF600 機型 profile；**第二批**＝建議配色演算法本體，
   **等待辦 2 修完再移**（否則把待查行為固定到客戶端）。
   ⚠ **推 `release/*` 會觸發 build_all CI（真 build）** ⇒ 碼可先 commit 在本地，**發車等 Eric 一句話**。
4. 🟠 **照片磚工作室自動安裝機型**（Eric 0807 需求）：從照片磚入口進去時，未安裝就自動勾——
   **四色→FF800＋FF600 同進照片磚；雙色→FD300 同進照片磚**。屬 C++（preset 安裝路徑）、**要 build**。
5. 🟡 **混色塊（洗料柱）與照片磚分開**（Eric 0807 裁）。⚠ 先釐清：**洗料柱 ≠ 色彩校正塊**
   （洗料柱＝`M6052 A25 B25 C25 D25` 四料等比同出的**單一顏色**柱，防久未用料空燒，量不出比例）。
   另 **`engine.js:565` 的洗料柱 `Z1 = P.height`＝跟磚一樣高**（實例 132.8mm ⇒ 25×25×132.8 實心柱
   ≈ 83cm³，比磚還耗料）——**要多高才擠得乾淨我不猜，等 Eric／工程端給實務值**。
6. 🟡 **建議配色 SSOT 統一**——Eric 裁「**等進版後再說**」。在統一前**改一邊必須改另一邊**。
7. ⚪ 承接自第八棒未辦：4GB 帽下切片 AV 硬崩／閘門收攤 `0xc0000374`／閒置預熱 332.5MB 要不要縮條件／
   閘門① 要不要用 warm 模式轉正（建議新 verdict `PASS_WARMED`）。

**🧰 本棒沉澱（不要只讀 handoff）**
- 🆕 **`SOP_開發線Portable試用包.md`**（新立）：組裝四步／`data_dir` 隔離機制與**實測驗法**／
  ASCII 別名 `D:\ping-slicer-c1` 與 junction 的關係／**`.bat` 一律 ASCII+CRLF**（Big5「會」尾碼
  `0x7C`＝`|` 會把 echo 行切成管線；UTF-8+`chcp` 會讓 cmd 誤剖後續位元組——兩種都實測崩過）／
  驗收表／**修正 SOP_WebView2 §18 一條：`taskkill /IM`（不加 `/F`）可優雅關閉本 app**，
  且不注入鍵盤 ⇒ 不污染離機哨兵，比 Alt+F4 更適合自動化。
- `照片磚_核心規格.md` **§6 色彩校正**（新增六條）。
- UX 筆記三條已 append 至 `D:\dev\2026claude\UX筆記收件匣.md`。

**線況**：開發線 `ping/v3.5`＝`53cc29bc`（bundle 78），**已推遠端**；
出貨線 `release/v3.6` **本棒全程未碰**；**不押 T**；
⚠ **但出貨線同日被別條線推進了**：本棒開工時是 `102fc948`，收工複驗＝**`b824a059`（bundle 91）**
——「0807 預勾線材補齊」那條 session 已進版並跑過 CI（見 `待確認/20260604 ORCA客製.md` 檔尾）。
⇒ **下一棒做最小刀前必須重抓出貨線 tip、重算兩線分歧數**（本棒記的「出貨線獨有 12 顆照片磚 commit」
是對 `102fc948` 算的），並先跟那條線協調（認領簿）。
`%APPDATA%` 與 Eric 正式版全程未碰（conf mtime 實測未動）；驗證實例零殘留。

---
### ✅ 建議配色核心規格＋FF600 照片磚・第八棒收工（2026-08-05 晚・牌 c-0805-PT-18／PT-19）

**一句話**：Eric 這一棒下了**三個定方向的裁示**（專攻垂直／配色規格立進文件／V3.6.1 但先在開發線試），
配色引擎的真兇找到並修好——**同一張照片色差 6.789→5.634、比人工手挑的還好**；FF600 四料照片磚機建好入版。

> 🧭 **下一棒起跑的三個前提，先讀這三行**：
> ① **產品線已切開**：垂直＝主線專攻；**平躺＝研究線、不推出**（Eric 0805 策略判準：平躺市場有 HueForge、
>    我們是後進者）。⚠ 平躺的模擬碼還活在 `index.html`（TD、Beer–Lambert 逐層合成），
>    **但從第一天起就沒有 3MF 輸出路徑——看到不要自己去接完**。
> ② **規格有正本了**：`照片磚_核心規格.md`（專案根目錄，長青）。動配色前先讀，別再從草案重推。
> ③ **開發線 bundle 已到 78**，且有 2 顆 commit **未推遠端**（見「線況」）。

**✅ 做了什麼**

**1. 建議配色核心規格落地（`b06216ac`，2 檔 +133/−28）**——本棒最大戰果
- **真兇是一行**：`engine.js:368`／`index.html:887` 的 `while(top.length<3) top.push(top[top.length-1])`
  ——色相家族不足三族時**複製最後一族**，人物照常只有「膚色/消色」兩族 ⇒ 第 3、4 槽吐出**逐字元相同**的 hex，
  四料當三料用。**第二個缺陷**：永遠取家族的 `deep`（最深那顆）⇒ 四支全擠暗端、中間調無代表。
- **一招治兩病**：改在**分群層做最遠點取樣**（種子＝權重最大群，之後取離已選最遠者，距離用 Lab 歐氏
  ＝與量化端指派同一把尺）。不同的群天生給不出同色；最遠點自動納入中間調。
  群數不足＝推亮度產真補色（**不複製**）＋末端 hex 去重保險。
- **實測**（quad／400×400mm／格點 3200²／K=8／同一張人物照）：

  | | 四支料 | 相異 | 色數 | ΔE |
  |---|---|---|---|---|
  | 舊 | `#f2f0eb/#070605/#2e231c/#2e231c` | 3 | 19 | 6.789 |
  | **新** | `#f2f0eb/#070605/#bfa08c/#533d30` | **4** | **34** | **5.634** |
  | 人工手挑（參考） | — | 4 | 36 | 6.251 |

  **新的自動配色比人工手挑的還好。** 對照：整個 K=6→14 掃描總增益僅 0.144，本刀 **1.155＝8 倍**。已眼驗渲染畫面。
- **另兩件**：堵掉 4 條靜默重新建議路徑（洗料柱／尺寸／雙四色／平躺垂直），只剩「載圖」與「主動按鈕」
  兩個覆寫入口；換料清單不再寫死「白」（客戶換掉料A 後現場會換錯料）。
- **黃金基準已重錄且逐案交代**：先帶舊基準跑一輪取證 ⇒ **六個 dual 案位元組一字未變（SHA 相同）、
  六個 quad 案 +11.9~12.1%**（`inputSha` 相同＝差異只來自演算法，證明改動只咬四料）；
  舊基準改名保留 `golden_manifest_preSuggestFix_20260805.json`；重錄後**再跑確認輪＝PASS 12/12、0 不符**。

**2. FF600 同進照片磚建好（`90a281b0`，bundle 77→78）**
- 三口徑（0.4/0.6/1.0）＋machine_model＋三支製程＋cover，`verify_profiles.py` **[OK] 全過**（315 presets／79 machines）。
- 做法照 SOP §2：範本＝FF800 同進照片磚，**delta 從「FF800 同進 vs FF600 同進」這對已知正確的機 diff 出來**，
  不手打。產法固化成 `tools/ping/build_ff600_phototile.py`（預設乾跑、內建三道 SOP assert）。
- **保留號段**（新機制）：插新 preset 會把後面批次的 `setting_id` 整批推移，實測打紅 verify 的 id baseline 18 條。
  **沒有鬆綁守衛**，改成從 `PINGM/PINGP 900` 起取號、不動共用計數器 ⇒ **既有 setting_id 變動行數 = 0**。
- ℹ **FF800 本來就是四噴頭機**，四料今天用它就測得了，不必等 FF600。

**3. C-2 第 5 項結案（`cf4bd983`／`c78a4813`／`9cb7a2d2`，第七棒同日稍早）**
- A＝排隊期取消不再被吞（雙時序實測 310ms／12ms）；第 6 項段界競態＋K48→K8。
- **第 5 項「先定罪」的結論是前提不成立**：「漂移 ~5 秒」實為冷啟動窗、`passDrift` 從沒紅過；
  被點名的 SHA 只佔 13%，真正大宗是 JSON 剖析 51%。**Eric 裁①不改、結案**，儀器留在閘門③當迴歸網。

**🟠 交下一棒的待辦（依重要度）**
1. 🔴 **2 顆 commit 未推遠端**（`b06216ac`、`90a281b0`）——遠端停在 `9cb7a2d2`。推不推等 Eric 一句話。
2. 🔴 **開發線 Portable 試用包還沒組**（Eric 0806 令「先在開發線的 Portable 測試，我再進版」）。
   組裝清單已備齊：build 樹 32 個 exe/dll（含 `WebView2Loader.dll`，SHA 與正式包相同）
   ＋`resources` 2906 檔（robocopy `/E /XJ` 解 junction）＋從 `D:\PING-Slicer-portable` 補 8 支 VC++ runtime
   ＋種入 `data_dir` 隔離資料夾。**⚠ 必須自帶 `data_dir`**（`GUI_App.cpp:2511-2514`），
   否則會去讀寫 Eric 正式版的 `%APPDATA%\PingSlicer`（bundle 89、DL1016 只活在那裡）。
   ⚠ **EBWebView 暫存目錄兩邊共用且導不走 ⇒ 不可與正式版同時開**。
3. 🟠 **V3.6.1 進版**（Eric 已裁版號 3.6.1、高流量加入、照片磚「釐清後在本地跑完再加」）。
   ⚠ **出貨線沒有 `engine.js`**（還是舊的單檔架構），照片磚相關碼兩線差 42 顆 commit
   ——「把照片磚加進去」**不是 cherry-pick 一顆**，要先評估整合方式。
4. 🟡 **平躺↔垂直切換那條沒單獨行為實測**（與另外三條同一種改法，但只驗了三條）。
5. 🟡 **建議配色仍有兩份實作**（`engine.js suggestSlots()` 與 `index.html suggest()`，客戶按鈕走後者）。
   本棒同步改了，**統一為單一來源另案**；在統一前**改一邊必須改另一邊**。
6. 🟡 **FF800 照片磚在選機頁只列出 0.4mm**（Eric 截圖），但 bundle 有 0.4/0.6/1.0 三個
   ——可能只是對話框高度截斷，**未查證**。
7. ⚪ 未辦技術債：4GB 帽下切片 AV 硬崩（已查出根因＝`OrcaSlicer.cpp` 的 new_handler 故意 null-deref，
   使兩條 `catch(std::bad_alloc)` 人話訊息成為死碼；⚠ 0803 那次的「OOM」歸因**從未被證明**，
   當時切的是 FF800 同進＝有 T 索引越界前科的機型家族）／閘門收攤 `0xc0000374`。

**🧰 本棒沉澱到 SOP（不要只讀 handoff）**
- `SOP_加機型.md` 新增 §2.7 派生新機用 diff 取 delta 不手打／§2.8 保留號段（插 preset 會推移既有 id，
  **不可為此鬆綁守衛**）／**§2.9 查 CJK 檔名必須關 `core.quotepath`**（本棒又踩一次，0726 已踩過）。
- `SOP_WebView2隱形宿主與跨層協定.md` §10 新增四條量測紀律（max 型指標量不到累積佔用等）。
- `照片磚_核心規格.md`＝**新立的長青正本**（產品定位／R1 白／R2 不得複製充數／R3 分散是目標不是閘門／
  §2 這是建議引擎規格不是使用者限制／退化階梯／驗收判準）。

**📄 本棒產出的檔案**
`ORCA專案現況_20260805.html`（全專案盤點）／`照片磚_K階測試/照片磚K階實測_20260805.html`（含人物範例圖與各 K 對照）／
`照片磚_色彩校正/色彩校正_白黑|白橘_FD300_0.4.3mf`（Eric 提的實印校正片，**尚未印**）＋`make_calib_3mf.py`。

**線況**：開發線 `ping/v3.5`＝`90a281b0`（bundle 78），**遠端停在 `9cb7a2d2`、2 顆未推**；
出貨線 `release/v3.6` 全程未碰（bundle 90）；**不押 T**；`%APPDATA%` 與 Eric 正式版全程未碰。
...
- ✅ **Eric 0805 已裁＝①「不改，結案」**——**C-2 第 5 項到此為止，不要再有下一棒去動 result 路徑**。
  儀器留在閘門③當迴歸網（哪天 `xferUiTotalMs`／佔用率飆高就會自己現形）。
  下面三選一留作裁決紀錄與日後翻案的依據。
- 🟡 **當時交 Eric 裁的三個選項**（越下面越貴）：
...
**🚩 下一棒（Eric 0805 兩裁已下）**
- ✅ **第 5 項＝①不改、結案**（見上）。**C-2 六項到此全部收完或結案**。
- 未辦（都還沒排）：第 4 項②「切片在 4GB 帽下 AV 硬崩」＝app 級大工程、與照片磚無關；
  「閘門收攤 `0xc0000374`」＝只咬閘門自己、要燒兩次 build 做 A/B 才能定罪。
- 仍等 Eric 的兩題（承接自 PT-16，不擋任何事）：①閒置預熱記憶體 332.5MB 要不要縮條件
  ②閘門① 要不要用 warm 模式轉正（建議新 verdict 名 `PASS_WARMED`）。
- **線況**：開發線 `ping/v3.5`＝`c78a4813`，✅ **已推遠端**（Eric 0805 裁「推」；
  `dafac44a..c78a4813`、`ls-remote` 複驗一致）。**不押 T**；出貨線 `release/v3.6` 全程未碰。
  ℹ 推這條**不會觸發 CI**——`build_all.yml` 只在 `main` 與 `release/*` 上跑（推前實查）。
...
**線況**：開發線 `ping/v3.5`＝`5ab6cee8`（含 `3f7af9ce` 修碼），**已推遠端**（Eric 0804 晚裁「推」；
`b905fd9f..5ab6cee8`、`ls-remote` 複驗一致）。
ℹ **更正**：下方第三棒段寫「55fb90f5 未推遠端」已過時——SP-05 棒 0804 已依 Eric 令連帶推出。
...
3. ~~下顆 T 發車~~ ✅ **T016 已出貨上架**（四批全在車上）；**D 槽已換裝 T016**（2026-07-31 Eric 令；驗證九項全過、備份留 T013+T015）。**剩 Eric 開 app 眼驗五項**（版本資訊列明；C-12 行為＝升級後記住的選擇不掉、切機亦保留）。
...
**本棒五大成果**：
1. **T015 全鏈出貨完成**（run 30496218297 @ `a7d95bad`、bundle 86）：Windows 主建置綠＝C++ 三批（GCode 認 M6050／Tab 連動表＋#39 守衛／PresetComboBoxes）＋新閘門 `ping_verify` CI 首驗全過。成品驗收＝同代 verify 389/108 打在下載 bundle＋**四批抽驗全過**（牆加速度 1500×358・Classic 隔離 0×64／洗料塔 cone×211・角30・速60／**Classic 32 機 start 零帶 T・DUAL 本體 4 台 M6050 換料模板・變體零誤帶**／五類名各×18・舊 token 0・renamed_from 90/90）＋**反向測試三注三中**。已上架 G 槽（installer `8363f5b9…a0a4`／portable `10d4bc1a…7599`、hash MATCH、T014 輪替 `old\`）。
2. **D 槽已換裝 T015**（Eric 令）：T013 備份 `-old-v360T013-0730`→解壓 44 項→DL1016 重注→驗證全過→備份輪替留 T012+T013。
3. **conf 組合名手術完成**（Eric 令「手術前計劃跟 Codex 雙 AI 審查」）：計畫 v1 **被一輪否決**→**六輪對抗審**逐輪收斂（二輪起 Codex 每輪親自故障注入）→六輪「可定稿」→ Eric 核可 P-4 殘窗＋過目清單→**apply 一次過**（token `b8c8170686a826c6`、14 種 20 處、腳本內三重復驗＋腳本外獨立複驗五項全過＋冪等確認、備份 `.bak-comborename-20260730124159064`）。腳本 v6＝兩線 `dd84cccd`/`dddc79ed`。
4. **兩個參數批入版（押下顆 T）**：①**高流量家族回抽 3/30/30**（`8d61d7b6`/`100403fb`、bundle 87/74）——實查證實 Eric 判斷屬實且更漏一層（四料高流量兩支的 speed/deretraction 原為 nil＝繼承機器 20/20）；範圍 7 支含 3in1。②**牆體列印方向全庫固定 ccw**（`b6525b0a`/`5389f846`、bundle 88/75）。
5. **圓孔處紋路帶研究**（Eric「驗」）：**首次實證 CLI 無頭切片可用**→造測試件跑六組參數矩陣→逐層 gcode 分析→交 Codex 技術討論。

**🔴 下一棒最優先**：
1. **高流量製程組**（Eric 已裁定、尚未動工）：客戶（建誌）FF800 同進參數 → 建「高流量」製程組。**定案值**＝六類速度全跟客戶（外牆100／內牆125／填充150／頂面150／實心150／支撐100）＋**首層也開 100**（Eric 追裁，推翻我建議的保留 40）＋**加速度逐項取保守**（列印 1500、空跑 2000、外牆 1500；填充/頂面/首層取兩者較小）＋層高 0.3／首層 0.35＋頂底厚 1.2＋**支撐角維持 35**（客戶的 Cura 60 換算 Orca 是 30，但 35 是 PING 實測收斂值）＋填充圖樣/接縫/溫度全維持 PING 現值。範圍＝FF800 同進三口徑（0.4/0.6/1.0），0.6 用客戶實測值、另兩個照口徑連動推。命名建議 `0.3mm 高流量 @FF800 同進 (0.6)`（已確認 `combo_token()` 非五類回 None、不會誤判 verify）。巡檢報告＝`FF800高流量參數_巡檢報告_20260730.html`。
2. **C-12 產品級缺口**（Eric 裁「做」）：升級 T015 的**其他機器** conf 記的舊組合製程名會 fallback 掉選擇——引擎 `select_preset_by_name_strict()` 是 exact-only、無 renamed 回溯（`PresetBundle.cpp:1979`→`Preset.cpp:3148` 實查）。修法＝C++ 在載入 `orca_presets` 的 process/filament 選擇時，strict 失敗改先過 renamed resolver 再選；兩線對稱＋verify 跨層護欄，押下顆 T CI 首驗。
3. **下顆 T 發車**（車上已有：回抽 3/30/30＋牆方向 ccw＋待加的高流量製程組＋C-12 C++）。
4. **紋路問題實印驗證**：E5 候選參數（`_研究_圓孔紋路_E5候選參數_20260730.json`）——生產只改四項：`bridge_speed=60`／`bridge_acceleration=100%`／`bridge_flow=1.0`／overhang 四級速度=60。gcode 層已證速度流量落差被拉平（孔帶最慢 960→1680），**紋路是否消失必須實印**。
5. **DUAL 分頁疑問待判別**（Eric 問）：資料層正確（12 台 DUAL 全 `family=Classic`），但 UI 有「照片磚」分頁而資料無此 family ⇒ 分頁非純讀 family。**判別法：切 Classic 分頁往下滾看 DUAL 在不在**——在＝長清單+跳轉錨點（無 bug）、不在＝分頁分類 bug 要挖 UI 碼。

**知識沉澱（本棒新增／擴充）**：
- 🆕 `SOP_無頭切片與gcode分析.md`（新檔）＝**CLI 切片實證用法＋五個踩坑＋gcode 分析三大陷阱**（`;TYPE:` 分段≠拓撲／點數門檻會靜默濾掉矩形／開放路徑不能算有號面積）＋參數矩陣紀律。
- `SOP_參數入版紀律.md` §8~§13（本棒補六條）：全庫值變更的基底判準／Classic 前綴識別／**conf 尾行 MD5＋文字手術**／**真實樣本冪等測**／user preset inherits 有 renamed 回溯但 C++ 硬寫名沒有／**動使用者實檔的腳本＝v6 樣板**。
- 審查全軌跡 21 檔＝專案根 `_審查_conf組合名手術_*`（計畫 v1~v6＋Codex 六輪＋回審五檔＋測試套）；紋路研究 4 檔＝`_研究_圓孔*`。
...
**✅ C-0 已結案——Eric 2026-07-30 裁「准 B＋豁免」**（原話）：①**准 B（隱形引擎）進 C-1**，帶三閘門②**真實睡眠喚醒豁免出 C-0**、列 C-1 過夜驗證。以下＝**C-1 起跑包（下一棒／新 session 讀這段就能開工）**：

- **先讀**：`..\照片磚_C-0尖峰報告_20260729.html`（§2.1 引擎契約／§3.3 PhotoTileEngineHost 規格草案／§6 退出條件／§7 殘餘風險）＋`..\照片磚_C0尖峰產物\README.md`（重現步驟）＋計畫正本 `..\照片磚_C案準備頁整合計畫_20260729.html`（§3 階段表、§5 十一項裁決全已拍板）。
- **C-1 範圍**：job 協定＋`PhotoTileEngineHost`（§3.3 草案為實作依據）＋最小 metadata schema（第 6 項裁決＝原圖嵌 3MF、save/reopen 續調）。起點碼＝`照片磚_C0尖峰產物\phototile_engine.js`（已與工作室雙路 oracle 6/6 全等）。
- **🔴 首站三閘門（任一敗＝觸發轉 A 評估，不得跳過）**：①最小 wx 整合 smoke（**此時才 build C++**，量 UI 事件圈延遲）②**實機睡眠過夜驗證**（本裁豁免出 C-0 的補償閘門）③OOM／低記憶體 gate（低規機 GRID_MAX 降階＋decoded-pixels 上限）。
- **B 案前提五項（C-1 必做、非可選，報告 §6）**：引擎頁生成路徑**零計時器**紀律（隱形頁 setTimeout 被節流 ~1s）／專用 host 不沿用 WebViewPanel／runtime 檢測前置＋誠實不可用（第 8 項裁決）／圖片編碼分塊移出 UI 執行緒／**進度顯示＋取消**（合法上限 quad K=48 ≈1 分鐘）。
- **退出條件四條**（WebView2 升級回歸收不了／跨層除錯失控〔單季 ≥3 件歸因逾日〕／效能 >10s／OOM 且降階不可接受）＝觸發即回報 Eric 裁轉 A；轉 A 成本已預壓（engine-neutral DTO＋雙路黃金基準庫＝介面不變、驗收現成）。
- **⚠ build 時機約束**：`build_all.yml` concurrency `cancel-in-progress: true`（同分支後推砍前 run）⇒ **T016 build（run 30550388975）未完成前不得推出貨線**；wx smoke 優先走本地編譯。
- **後續仍會回頭找 Eric 的**：P2A 縫遮罩轉預設開（第 7 項裁決＝實印終驗後另裁）／三閘門任一敗的轉 A 裁決／低規機 GRID_MAX 降階策略。
